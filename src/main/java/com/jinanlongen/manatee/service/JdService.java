...
public class JdService {
  // private static Logger logger = LoggerFactory.getLogger(JdService.class);
  ...
  public String updateCategory(Long cid, String shop_id) {
    ...
    // 颜色尺码属性
    List<BigInteger> attrId = CARep.findOtherAttrByCidAndShopid(cid, shop_id);
    System.out.println("--" + attrId.size());
    saveAttribute(attrId, shop_id, "ColorAndSize");
  ...
  // http://jos.jd.com/api/complexTemplate.htm?webPamer=categoryAttr&groupName=%E7%B1%BB%E7%9B%AEAPI&id=62&restName=jingdong.category.read.findAttrByIdJos
  ...
  public String synOneShop(String id) {
    ...
    if (categoryList == null || categoryList.size() == 0) {
      ...
      for (Category jdcategory : categoryList) {
        // 商店类目对应表
        JdShopCategory shopMap = new JdShopCategory();
  ...
  public void shoptest() {
    ...
    Shop shop = shopRep.findOne("23");
    JdUtils jdu = new JdUtils(shop);
    List<Category> categoryList = jdu.get();
    if (categoryList == null || categoryList.size() == 0) {
      System.out.println("wuleimu");
    } else {
      for (Category jdcategory : categoryList) {
        // 商店类目对应表
        System.out.println(jdcategory.getId());
        System.out.println(jdcategory.toString());
